...
import { deleteRolModel, getAdminNotProviderModel, getIdRolesModel, getRolProviderModel, getRolesModel, getRolesNameModel, postRolesModel, putRolModel } from '../models/roles.model';
...
// TRAER NOMBRE ROLES
export const getRolesName = async (req: Request, res: Response) => {
    const { api_key } = req.headers;
    try {
        if (apiKeyValidate(api_key)) return res.status(401).json(unauthorized());
        return res.status(200).json(success((await getRolesNameModel()).data));
    } catch(error) {
        return res.status(512).json(unsuccessfully(error));
    }
};
