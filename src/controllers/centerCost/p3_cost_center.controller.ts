...
export const getCostCenterById = async (req: Request, res: Response) => {
    ...
    try {
        if (apiKeyValidate(api_key)) return res.status(401).json(unauthorized());
        if (missingDataObject({idcost_center_subarea}).error) return res.status(422).json(uncompleted(missingDataObject({idcost_center_subarea}).missing));
...
export const deleteCostCenter = async (req: Request, res: Response) => {
    ...
    try {
        if (apiKeyValidate(api_key)) return res.status(401).json(unauthorized());
        if (missingDataObject({cost_center}).error) return res.status(422).json(uncompleted(missingDataObject({cost_center}).missing));
