...
export const postFile = async (req: Request, res: Response) => {
    ...
    try {
        if(nullValidator(valores)) {
            return res.status(500).json({message: "ERROR_MISSING_VALUES"});
        };
        const [ radValidate ] = await connection.query(`
                SELECT count(*) AS contador FROM files WHERE files_registered = ?;`,
            [ files_registered ]);
        //@ts-ignore
        if (radValidate[0].contador !== 0){
            return res.status(500).json({message: `Radicado: ${ files_registered }, ya se encuentra creado en la base de datos`})
        };
...
export const putFile = async ( req:Request, res:Response ) => {
    const { idfiles, 
            ...
            userSession, tracking_observation } = req.body;
    ...
    try {
        ...
        if ( data[0].contador === 0 ) {
            return res.status(404).json({ message: `Archivo con el id: ${ idfiles } o radicado: ${ files_registered }, no se encuentra registrado en la base de datos` })
        } else {
            await connection.query(`
                UPDATE files SET
                    idproviders = ?, 
...
export const deleteFile = async (req:Request, res:Response) => {
    ...
    try {
        if( api_key != process.env.API_KEY) {
            return res.status(401).json({ message: "No cuentas con los permisos para eliminar un archivo" })
        };
        if (nullValidator(values)) {
            return res.status(500).json({message: "ERROR_MISSING_VALUES"});
        };
